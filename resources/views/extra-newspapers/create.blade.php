    ...
    <x-slot name="header">
        <div class="flex items-center space-x-2">
            ...
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ __('{{ __('messages.add_extra_newspaper') }}') }}
            </h2>
        </div>
    </x-slot>
    ...
    <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden mx-auto">
        <form action="{{ route('extra-newspapers.store') }}" method="POST" class="p-6">
            ...
            <div class="mt-6 flex justify-end">
                ...
                <button type="submit"
                    class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700">{{ __('messages.save_extra_newspaper') }}</button>
            </div>
        </form>
    </div>
