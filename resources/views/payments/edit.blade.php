<x-app-layout>
    ...
    <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden mx-auto">
        <form action="{{ route('payments.update', $payment->id) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
                    ...
                    <select name="invoice_id" id="invoice_id"
                        ...
                        @foreach($invoices as $invoice)
                            <option value="{{ $invoice->id }}" data-customer="{{ $invoice->customer_id }}"
                                data-due="{{ $invoice->total_amount - $invoice->paid_amount }}" {{ old('invoice_id', $payment->invoice_id) == $invoice->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="amount" id="amount"
                        value="{{ old('amount', $payment->amount) }}" min="1"
                        class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 rounded-md shadow-sm"
                        required>
                ...
                <div>
                    ...
                    <select name="payment_mode" id="payment_mode"
                        ...
                        <option value="UPI" {{ old('payment_mode', $payment->payment_mode) == 'UPI' ? 'selected' : '' }}>
                            UPI</option>
                        <option value="Bank Transfer" {{ old('payment_mode', $payment->payment_mode) == 'Bank Transfer' ? 'selected' : '' }}>Bank Transfer</option>
            ...
            <div class="mt-6">
                ...
                <textarea name="notes" id="notes" rows="3"
                    class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 rounded-md shadow-sm">{{ old('notes', $payment->notes) }}</textarea>
