<x-app-layout>
    ...
    <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden mx-auto"
        x-data="{ deliveryType: '{{ old('delivery_days', $subscription->delivery_days) }}' }">
        <form action="{{ route('subscriptions.update', $subscription->id) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
                    ...
                    @foreach(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'] as $day)
                        <label class="inline-flex items-center">
                            <input type="checkbox" name="custom_days[]" value="{{ $day }}"
                                ...
                                {{ is_array($customDays) && in_array($day, $customDays) ? 'checked' : '' }}>
